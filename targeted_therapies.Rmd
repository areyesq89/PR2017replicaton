...
The main finding of Haibe-Kains et al. (2013) was that the genomic profiles of the cell lines used in both studies were highly correlated. What you might have found so far in your explorations of comparing the summary measures of drug response is that the agreement between the studies does not seem as highly reproducible. Part of this may be explained by differences in protocol (which you can see in the different drug concentrations tested in the two studies). In the rest of this vignette, we'll investigate further sources of variation and biological mechanisms that might explain some of this lack of agreement.
...
Something that was not considered in the original comparison analyses by Haibe-Kains et al. was that different cell lines may more or less **resistant** (resistance means that the cell line does not respond to treatment by a drug) to drug treatment than others. Let's explore first why that might be an important factor, and then directly compare cell line sensitivity in the two studies. 
...
What would the IC50 value be in this case? 
...
Find 5 cell lines which seem the most resistant in both studies according to the average IC50.
...
Find 5 cell lines which seem the most resistant in both studies according to the average AUC.

> Place your answer here

Are the answers above consistent for both IC50 and AUC?
...
To assess agreement between the studies on which cell lines were sensitive or resistant, we first have to define criteria for deciding whether a cell line is resistant or not. In other words, we'd like to choose an AUC cutoff that divides sensitive and resistant responses. This choice is somewhat arbitrary, but for now we'll choose the values used by the Safikhani et al. paper that explored this concept: AUC values 0.2 or higher indicate that the cell line is sensitive to the drug, and AUC values below 0.2 indicate that the cell line is resistant (unless no cell lines had observed AUC values above 0.2 in either study, then use 0.4. This is the case for the broadly cytotoxic drug paclitaxel). 
...
cellLinesSummary <- cellLinesSummary %>% 
              mutate(cutoff = ifelse(drug=="paclitaxel", 0.4, 0.2)) %>%
              mutate(sensitivity_GDSC = factor(ifelse( auc_GDSC < cutoff, "Sensitive", "Resistant")), 
                     sensitivity_CCLE = factor(ifelse( auc_CCLE < cutoff, "Sensitive", "Resistant"))) 
...
Without getting into too much detail about the statistic, let's go ahead and see how it is calculated, and compare its values across all the drugs. We'll first have to load the `PharmacoGX` package which contains the `mcc` function to calculate the MCC. Note that if you don't have this package installed on your system, you can first install it using the Bioconductor commands below.
...
1. Are there any cell lines that seem to be either (1) consistently sensitive or (2) consistently resistant? What effect might this have on your conclusions and interpretations of the agreement between the studies? For example, do you observe any increase in replicability (as measured by correlation of drug response) if you exclude the most resistant cell lines?
