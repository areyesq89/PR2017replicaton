...
```{r, AUC resistant}
AUC_study1 <- rbeta(200, 1, 25)
AUC_study2 <- rbeta(200, 1, 25)
resistant <- data.frame(AUC_study1, AUC_study2)
ggplot(resistant, aes( y=AUC_study2, x=AUC_study1) ) +
    geom_point() + main("Simulated AUC of resistant cell lines")
```

Here we have simulated the AUC values in each study to be close to zero but with some random variability (this is done here using the Beta distribution). Do these AUC values look correlated? Calculate a correlation coefficient to quantify the correlation.
...
```{r, ic50 by cell line}
ggplot(cellLinesSummary, aes( y=ic50_GDSC, x=drug, fill=study) ) +
    geom_boxplot() +
 #   facet_grid(study~.) +
