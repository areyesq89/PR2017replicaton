...
regression. If you have interest in knowing more about either
...
from the two drug-cell line combinations mentioned in the previous 
section. Ideally, we would like the regression model to be as 
...
values by predicting the drug concentration value of the 
logistic regression model that gives a viability score of 50%. 
...
In the same way, let's now calculate AUC values based on the
...
Lets compate the scores estimated using code from this vignette between the different studies.
...
```
## Modeling drug response using linear regressions.
...
Below is an example on how to use this function and how to extract
the slope of the linear model

```{r}

linearModelCCLE1 <- fitLinearModel( "17-AAG", "H4", "CCLE" )
coefficients( linearModelCCLE1 )["concentration"]
linearModelGDSC1 <- fitLinearModel( "17-AAG", "H4", "GDSC" )
coefficients( linearModelGDSC1 )["concentration"]
